...
const BookingAppointmentPage = () => {
    ...
    const validate = () => {
        ...
        else if (new Date(form.appointment_date) < new Date(new Date().toDateString())) errs.appointment_date = 'Ngày hẹn không hợp lệ';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const selectedSvc = services.find(s => s.service_id === form.service_id);
    ...
    const SelectField = ({ label, field, options, placeholder, renderOption }) => (
        <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">{label} <span className="text-red-400">*</span></label>
            <select
                value={form[field]}
            ...
            {errors[field] && <p className="text-red-500 text-xs mt-1.5 font-medium flex items-center gap-1"><i className="fa-solid fa-circle-exclamation text-[10px]"></i>{errors[field]}</p>}
        </div>
    );
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border border-blue-200 mb-1">
                                ...
                                <span className="text-[10px] font-bold text-sky-700 uppercase tracking-wider">Đặt lịch</span>
                            </div>
                            <h1 className="text-2xl font-extrabold text-gray-900">Đặt lịch khám</h1>
                        </div>
                    </div>
                </div>
            </div>
                ...
                >
                    ...
                    <div className="p-6 space-y-5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-2">Ngày khám <span className="text-red-400">*</span></label>
                                ...
                                {errors.appointment_date && <p className="text-red-500 text-xs mt-1.5 font-medium flex items-center gap-1"><i className="fa-solid fa-circle-exclamation text-[10px]"></i>{errors.appointment_date}</p>}
                            </div>

                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-2">Giờ bắt đầu <span className="text-red-400">*</span></label>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                ...
                                {errors.start_time && <p className="text-red-500 text-xs mt-1.5 font-medium flex items-center gap-1"><i className="fa-solid fa-circle-exclamation text-[10px]"></i>{errors.start_time}</p>}
                            </div>
                        </div>
                    </div>

                    <div className="px-6 py-5 border-t border-gray-100 flex flex-col sm:flex-row justify-end gap-3">
                        <button type="button" onClick={() => navigate('/patient/dashboard')} className="px-6 py-3 rounded-xl font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-all active:scale-95 cursor-pointer">
                        ...
                        >
                            {submitting ? (
                                ...
                                <><i className="fa-solid fa-calendar-check"></i> Đặt lịch & Thanh toán</>
                            )}
                        </button>
                    </div>
                </motion.form>
